...
class RepositoriesListAdapter(private val repositoryList: ArrayList<RepositoryModel>, val itemClickListener: OnItemSelectedListener) : RecyclerView.Adapter<RepositoriesListAdapter.RepositoryItemViewHolder>() {
    ...
    class RepositoryItemViewHolder(itemView: View, private val itemClickListener: OnItemSelectedListener) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bindData(repositoryModel: RepositoryModel) {
            ...
            itemView.tv_repository_updated_time.setText(repositoryUpdateTime)
            // itemView.setBackgroundColor(if (hasWiki) ContextCompat.getColor(context!!, R.color.colorPrimary) else ContextCompat.getColor(context!!, R.color.colorPrimaryDark))
